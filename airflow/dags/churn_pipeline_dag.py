...
Orchestrates the daily churn-prediction pipeline:
    ...
    wait for raw extract -> ingest -> dbt build (run+test) -> score customers
...
Drop this file into your Airflow `dags/` folder. Assumes:
    ...
    - two Airflow Variables are registered: `dbt_target` (e.g. "prod") and `slack_alerts_webhook` (optional -- 
    failures just go unnotified without it, which is fine for local/dev)
...
with DAG(
    ...
    wait_for_raw_data = FileSensor(
        ...
        filepath="/opt/airflow/data/raw/{{ ds }}/_SUCCESS",
        fs_conn_id="fs_default",
        mode="reschedule",  # frees the worker slot instead of blocking it while polling
        timeout=60 * 60 * 3,
        poke_interval=60 * 5,
        doc_md=(
            "Waits for the upstream extract job to drop a `_SUCCESS` marker "
            "for the run date before we touch anything. Swap for a GCS/S3 "
            "sensor if raw data actually lands in cloud storage rather than "
            "this mounted volume."
        ),
    )
    ...
    ingest = BashOperator(
        ...
        doc_md=(
            "Loads the day's raw Olist extract into the BQ raw dataset. "
            "Idempotent on `--date`, safe to rerun or backfill."
        ),
    )
    ...
    dbt_build = BashOperator(
        ...
        doc_md=(
            ...
            "checked. `--fail-fast` stops at the first failure instead of "
            "grinding through a run that's already doomed."
        ),
    )
    ...
    score_customers = BashOperator(
        ...
        doc_md=(
            ...
            "weekly training DAG -- never trains one itself."
        ),
    )

    wait_for_raw_data >> ingest >> dbt_build >> score_customers